...
void readGarden(FILE* fp, Garden* pGarden, int opt) {
	...
	if (opt == 0) {
		...
		type = 0;
		fread(&count, sizeof(int), 1, fp);
		fread(&sTemp, sizeof(char) * count, 1, fp);
		fread(&type, BYTE, 1, fp);
		pGarden->childCount = (type >> 2);
		type = type & 0X3;
	}
	...
	for (i = 0; i < pGarden->childCount; i++) {
		...
		if (!checkAllocation(pGarden->childPtrArr[i]))
			return;
		readChild(fp, pGarden->childPtrArr[i], opt);
	}
...
void showAllGardens(Garden** pGardenList, int count) {
	...
	printf("\n");
	if (count > 3) {
		vardicFunction(pGardenList[0]->name, pGardenList[0]->childCount,
...
void writeGardensToFile(Garden** pGardenList, int gardenCount, char* fileName,
	...
	for (i = 0; i < gardenCount; i++)
		writeGarden(fp, pGardenList[i], opt);
	fclose(fp);
}
...
void writeGarden(FILE* fp, const Garden* pGarden, int opt) {
	...
	if (opt == 0) {
		...
		int len, typeAndChildCount = 0;
		len = strlen(pGarden->name) + 1;
		fwrite(&len, sizeof(int), 1, fp);
		fwrite(pGarden->name, sizeof(char) * len, 1, fp);
		typeAndChildCount = (pGarden->type) | (pGarden->childCount << 2);
		fwrite(&typeAndChildCount, BYTE, 1, fp);
	}
	for (i = 0; i < pGarden->childCount; i++)
		writeChild(fp, pGarden->childPtrArr[i], opt);
}
...
int compareByName(const void* garden, const void* garden2) {
	const Garden* g1 = *(Garden**) garden;
	const Garden* g2 = *(Garden**) garden2;
...
int compareByTypeAndNuberOfChild(const void* garden, const void* garden2) {
	const Garden* g1 = *(Garden**) garden;
	const Garden* g2 = *(Garden**) garden2;
	int type1 = (int) (g1->type);
	int type2 = (int) g2->type;
	if (type1 > type2)
		return 1;
	if (type1 < type2)
		return -1;
...
int choseGraden(const int numOfGarden) {
	int number;
	printf("please insert number of garden to search");
	scanf("%d", &number);
	number = number - 1;
	if (number >= numOfGarden || number < 0) {
		printf("please chose illegal number");
		return -1;
	}
	return number;
}
